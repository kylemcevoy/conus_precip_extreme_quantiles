...
output_dir = synthLE_dir + 'analysis/q95/djf/'
...
synth_q95_sd = synth_q95_sd.where(nan_mask)

synth_means = synth_q95.groupby('cesm2_mem').mean('ens_mem')
synth_overall_mean = synth_means.mean('cesm2_mem')

synth_q95_mean_sd = synth_q95_sd.mean('cesm2_mem')
synth_q95_sd_sd_djf = synth_q95_sd.std(ddof=1, dim='cesm2_mem')
synth_q95_sd_sd_djf = synth_q95_sd_sd_djf.where(nan_mask)

local_sd_djf = synth_q95_sd.sel(lat=34.5, lon=360-118.5, method='nearest').compute()

local_mean_djf = synth_means.sel(lat=34.5, lon=360-118.5, method='nearest')
local_mean_djf = local_mean_djf.compute()

local_lon2 = 278.75
local_lat2 = synth_q95_sd.lat.values[4]
local_sd2_djf = synth_q95_sd.sel(lat=local_lat2, lon=local_lon2)

cesm2LE_q95_djf = cesm2_LE_djf.quantile(q=0.95, dim='time')

cesm2_q95_mean = cesm2LE_q95_djf.mean('ens_mem')

cesm2LE_q95_djf_sd = cesm2LE_q95_djf.std(ddof=1, dim='ens_mem')

local_cesm2_mean_djf = cesm2_q95_mean.sel(lat=34.5,
                                          lon=360-118.5,
                                          method='nearest')
local_cesm2_sd_djf = cesm2LE_q95_djf_sd.sel(lat=34.5,
                                            lon=360-118.5,
                                            method='nearest')

local_cesm2_sd_djf2 = cesm2LE_q95_djf_sd.sel(lat=local_lat2,
                                            lon=local_lon2)
...
frac_change_in_mean = ((synth_overall_mean - cesm2_q95_mean) 
                     / cesm2_q95_mean)

mean_mean_change = frac_change_in_mean.mean().compute().values

frac_change_in_sd = ((synth_q95_mean_sd - cesm2LE_q95_djf_sd) 
                     / cesm2LE_q95_djf_sd)
...
sd_cmap = cm.viridis
sd_norm = mpl.colors.BoundaryNorm(np.linspace(0, 10, 11), sd_cmap.N)
...
cbar.set_label('fractional change of mean s.d. [unitless]',
               fontsize=20)
ax1.coastlines()
ax1.set_title('')
...
ax1.annotate(f'mean: {mean_sd_change:.3f}', [280, 30], fontsize=20)
ax1.set_title('a)', loc='left', fontsize=20)

(synth_q95_sd_sd_djf).plot(ax=ax2,
                 cmap=sd_cmap,
                 norm=sd_norm,
                 add_colorbar=False)
cbar2 = fig.colorbar(cm.ScalarMappable(norm=sd_norm, cmap=sd_cmap),
                    ...
                    shrink=0.8,
                    orientation='horizontal')
cbar2.set_label('s.d. of quantile standard deviations [mm]',
               fontsize=20)
...
sns.histplot(local_sd_djf, ax=ax3, binrange=[10, 52], binwidth=3, color='tab:purple')
...
ax3.set_xlim((0, 60))
ax3.set_ylim((0, 18))
ax3.set_xlabel('s.d. of the 0.95 quantile [mm]',
               fontsize=20)
ax3.set_title('c)', loc='left', fontsize=20)
ax3.tick_params(axis='both', labelsize=20)

sns.histplot(local_sd2_djf, ax=ax4, binrange=[10, 52], binwidth=2, color='tab:cyan')
...
ax4.set_xlim((0, 60))
ax4.set_ylim((0, 18))
ax4.set_xlabel('s.d. of the 0.95 quantile [mm]',
               fontsize=20)
ax4.set_title('d)', loc='left', fontsize=20)
ax4.tick_params(axis='both', labelsize=20)

fig.savefig(plot_dir + 'sd_q95_djf_synthLE_cesm2LE_comp.png')
...
sns.histplot(local_sd_djf, ax=ax, binrange=[0, 36], binwidth=3, color='tab:purple')
ax.axvline(x=local_cesm2_sd_djf.compute().values, 
            color='red', 
            linestyle='--',
            label='CESM2-LE s.d.')
ax.axvline(x=local_sd_djf.mean().values, 
            color='black', 
            linestyle='--',
...
sns.histplot(local_sd2_djf, ax=ax, binrange=[0, 36], binwidth=2, color='tab:cyan')
ax.axvline(x=local_cesm2_sd_djf2.compute().values, 
            color='red', 
            linestyle='--',
            label='CESM2-LE s.d.')
ax.axvline(x=local_sd2_djf.mean().values, 
            color='black', 
            linestyle='--',
            label='synth-LE mean s.d.'
            )
...
sns.histplot(local_mean_djf, ax=ax2, binrange=[3.5, 6], binwidth=0.25)
ax2.axvline(x=local_cesm2_mean_djf.compute().values, 
            color='red', 
            linestyle='--',
            label='CESM2-LE mean q95')
ax2.axvline(x=local_mean_djf.mean().values, 
            color='black', 
            linestyle='--',
