...
gpcc_q95 = xr.open_dataarray(output_dir + 'gpcc_djf_q95.nc')
obsLE_q95 = xr.open_dataarray(output_dir + 'obsLE_djf_q95.nc')

gpcc_q95 = gpcc_q95.where(nan_mask)
obsLE_q95 = obsLE_q95.where(nan_mask)

obsLE_lb = obsLE_q95.quantile(0.025, dim='ens_mem').where(nan_mask)
obsLE_ub = obsLE_q95.quantile(0.975, dim='ens_mem').where(nan_mask)
ci_spread = obsLE_ub - obsLE_lb 

upper_spread = obsLE_ub - gpcc_q95
lower_spread = obsLE_lb - gpcc_q95

cmap_levels = np.linspace(0, 1200, 13)
cmap = cm.viridis
norm = mpl.colors.BoundaryNorm(cmap_levels, cmap.N, extend='max')

cmap_spread = cm.plasma
norm_spread = mpl.colors.BoundaryNorm(np.linspace(0, 150, 16),
                                      cmap_spread.N,
                                      extend='max')
...
norm_pos = mpl.colors.BoundaryNorm(np.linspace(0, 100, 11),
                                    pos_diff.N,
                                    extend='max')
norm_neg = mpl.colors.BoundaryNorm(np.linspace(-100, 0, 11),
                                    neg_diff.N,
                                    extend='min')
...
ax[0, 1].set_title('GPCC Observed DJF 0.95 Quantile', fontsize=font_size)
ax[0, 1].coastlines()
...
fig.savefig(plot_dir + 'gpcc_obsLE_djf_q95_CI.png')
...
ub_increase = ((obsLE_ub - gpcc_q95) / gpcc_q95)
